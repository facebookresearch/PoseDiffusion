...
class Re10KDataset(Dataset):
    def __init__(
        ...
        if cfg.debug:
            ...
            self.viz = vis_utils.get_visdom_connection(server="http://10.201.5.215", port=int(os.environ.get("VISDOM_PORT", 10088)))
        else:
            self.viz = None
    ...
    def get_data(self, index=None, sequence_name=None, ids=(0, 1), no_images=False, return_path=False):
        ...
        if self.normalize_cameras:
            ...
            norm_cameras, points = normalize_cameras(
                cameras, points=points, compute_optical=self.compute_optical, first_camera=self.first_camera_transform, normalize_T=self.normalize_T,
            )
